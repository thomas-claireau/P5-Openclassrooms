...
document.addEventListener('DOMContentLoaded', function (e) {
	...
	function startGenerator() {
		let choice = Number(
			prompt(
				'Choix du générateur de citation : \n1 : Générateur n°1 \n2 : Générateur n°2 \n0 : Arrêt du programme'
			)
		);
		...
		while (choice === 0 || choice === 1 || choice === 2) {
			...
			if (choice === 1) {
				choiceNumberQuotes = Number(
					prompt('Choisissez le nombre de citations (entre 1 et 5)')
				);
				...
				if (choiceNumberQuotes > 0 && choiceNumberQuotes <= 5) {
					...
					choice = Number(
						prompt(
							'Le nombre de citations générées doit être compris entre 1 et 5, voulez-vous recommencer ? \n1 : Générateur n°1 \n2 : Générateur n°2 \n0 : Arrêt du programme'
						)
					);
				}
			}
